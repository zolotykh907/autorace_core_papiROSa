...
from turn_robot.msg import Turn
...
class Intersection_Handler(Node):
    def __init__(self):
        ...
        self.enable_detection_pub = self.create_publisher(Bool, '/enable_detection', 1)
        self.max_vel_pub = self.create_publisher(Float64, '/max_vel', 1)
        self.offset_pub = self.create_publisher(Float64, '/offset', 1)
        self.rotate_pub = self.create_publisher(Turn, '/rotate', 1)
        ...
        if not self.rotated:
            ...
            if (cur_sign == 'turn_left_sign' or cur_sign == 'turn_right_sign') and self.enable_detection:
                ...
                if cur_sign == 'turn_left_sign':
                    ...
                    self.rotate_pub.publish(Turn(angle = self.angle_L, linear_x = self.linear_x_L, angular_z = self.angular_z_L, id = self.ID))
                    ...
                    self.rotate_pub.publish(Turn(angle = self.angle_R, linear_x = self.linear_x_R, angular_z = self.angular_z_R, id = self.ID))
